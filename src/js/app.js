App = {
  ...
  buyRealEstate: function() {
	...
	//ethereum.enable().then(function (accounts) { //연결시
    web3.eth.getAccounts(function(error, accounts) { //거래시
      if(error) {
        console.log('error');
      }
     ...
     App.contracts.RealEstate.deployed().then(function(instance) {
       ...
       for(i = 0; i < buyers.length; i++){
        if (buyers[i] !== '0x0000000000000000000000000000000000000000') {
          ...
          switch(imgType) {
			case 'house0.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house0_sold.jpg')
				break;
			case 'house1.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house1_sold.jpg')
				break;
			case 'house2.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house2_sold.jpg')
				break;
			case 'house3.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house3_sold.jpg')
				break;
			case 'house4.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house4_sold.jpg')
				break;
			case 'house5.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house5_sold.jpg')
				break;
			case 'house6.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house6_sold.jpg')
				break;	
			case 'house7.jpg':
				$('.panel-realEstate').eq(i).find('img').attr('src', 'images/house7_sold.jpg')
				break;
          }
...
$(function() {
  ...
  $('#buyerInfoModal').on('show.bs.modal', function(e) {
    ...
    App.contracts.RealEstate.deployed().then(function(instance) {
	  ...
	  $(e.currentTarget).find('#buyerNum').text(buyerInfo[3]);
	  $(e.currentTarget).find('#buyerIdNum').text(buyerInfo[4]);
    }).catch(function(err) {
      console.log(err.message);
    })
  });
